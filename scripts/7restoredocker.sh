...
restore_postgres() {
    ...
    $DOCKER_COMPOSE exec -T postgres psql -U postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${POSTGRES_DB:-tazagroupcore}' AND pid <> pg_backend_pid();" || true
    
    # Drop and recreate database
    log_info "Recreating database..."
    $DOCKER_COMPOSE exec -T postgres psql -U postgres -c "DROP DATABASE IF EXISTS ${POSTGRES_DB:-tazagroupcore};" || true
    $DOCKER_COMPOSE exec -T postgres psql -U postgres -c "CREATE DATABASE ${POSTGRES_DB:-tazagroupcore};"
