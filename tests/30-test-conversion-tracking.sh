...
echo "Recent clicks:"
echo "psql -d katacore -c \"SELECT id, \\\"linkId\\\", \\\"ipAddress\\\", device, browser, \\\"clickedAt\\\" FROM \\\"AffClick\\\" ORDER BY \\\"clickedAt\\\" DESC LIMIT 5;\""
echo ""

# Query to check conversions
echo "Recent conversions:"
echo "psql -d katacore -c \"SELECT id, \\\"orderId\\\", \\\"saleAmount\\\", commission, status, \\\"convertedAt\\\" FROM \\\"AffConversion\\\" ORDER BY \\\"convertedAt\\\" DESC LIMIT 5;\""
echo ""
