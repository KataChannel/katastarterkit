...
import { GET_ACADEMY_COURSES, GET_ACADEMY_COURSE_CATEGORIES } from '@/features/timona/graphql/queries';
import type { AcademyCourse, AcademyCourseCategory } from '@/features/timona/types';
    ...
    >
      ...
      <div className="p-6">
        ...
        {course.shortDescription && (
          <p className="text-gray-600 text-sm line-clamp-2 mb-4">{course.shortDescription}</p>
        )}
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          <div>
            {course.discountPrice ? (
              ...
              <span className="text-xl font-black text-[#00256e]">{formatPrice(course.price)}</span>
            ) : (
              <span className="text-lg font-bold text-[#00256e]">Liên hệ</span>
            )}
          </div>
...
function CoursesPageContent() {
  ...
  // Set active category based on URL param on initial load
  useEffect(() => {
    if (categorySlug && categories.length > 0 && !categoryInitialized) {
  ...
  }, [categorySlug, categories, categoryInitialized]);
      ...
      <div className="container mx-auto px-4">
        ...
        <nav className="mb-8">
          <ol className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <li>/</li>
            <li className="text-[#00256e] font-semibold">Khóa học</li>
          </ol>
        </nav>

        {/* Header */}
        <div className="mb-12">
          <h1 className="text-4xl font-black text-[#00256e] mb-4">TẤT CẢ KHÓA HỌC</h1>
          <p className="text-gray-600 text-lg max-w-3xl">
            Các khóa học được thiết kế bài bản, chuẩn quốc tế với đội ngũ giảng viên giàu kinh
            nghiệm
          </p>
        </div>
