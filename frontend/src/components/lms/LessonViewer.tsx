...
import { MARK_LESSON_COMPLETE } from '@/graphql/lms/courses.graphql';
import { GET_QUIZZES_BY_LESSON } from '@/graphql/lms/quizzes.graphql';
...
export default function LessonViewer({
  ...
  const [markComplete, { loading: markingComplete }] = useMutation(MARK_LESSON_COMPLETE);

  // Fetch quizzes for this lesson if it's a QUIZ type
  const { data: quizzesData } = useQuery(GET_QUIZZES_BY_LESSON, {
    variables: { lessonId: lesson.id },
    skip: lesson.type !== 'QUIZ',
  });
  ...
  const renderLessonContent = () => {
    switch (lesson.type) {
      ...
      case 'QUIZ':
        const quiz = quizzesData?.quizzesByLesson?.[0];
