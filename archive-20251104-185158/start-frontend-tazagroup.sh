...
# Start Frontend for Tazagroup Domain (Port 13000)

cd "$(dirname "$0")/frontend"

echo "🚀 Starting Tazagroup Frontend on port 13000..."
...
if [ ! -f ".env.local" ]; then
    ...
    echo "Creating .env.local with Tazagroup configuration..."
    cat > .env.local << 'EOF'
# Local Development Environment Variables for Tazagroup
