"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProjectMediaResolver = void 0;
const graphql_1 = require("@nestjs/graphql");
const common_1 = require("@nestjs/common");
const project_media_service_1 = require("./project-media.service");
const media_dto_1 = require("./dto/media.dto");
const current_user_decorator_1 = require("../auth/current-user.decorator");
const jwt_auth_guard_1 = require("../auth/jwt-auth.guard");
let ProjectMediaResolver = class ProjectMediaResolver {
    constructor(projectMediaService) {
        this.projectMediaService = projectMediaService;
    }
    async uploadProjectFile(projectId, filename, mimetype, size, url, caption, userId) {
        return {
            id: 'temp',
            type: 'DOCUMENT',
            url,
            filename,
            size,
            mimeType: mimetype,
            caption,
            projectId,
            uploadedBy: userId,
            createdAt: new Date(),
            updatedAt: new Date(),
        };
    }
    async uploadTaskFile(taskId, filename, mimetype, size, url, caption, userId) {
        return {
            id: 'temp',
            type: 'DOCUMENT',
            url,
            filename,
            size,
            mimeType: mimetype,
            caption,
            taskId,
            uploadedBy: userId,
            createdAt: new Date(),
            updatedAt: new Date(),
        };
    }
    async uploadChatFile(messageId, filename, mimetype, size, url, caption, userId) {
        return {
            id: 'temp',
            type: 'DOCUMENT',
            url,
            filename,
            size,
            mimeType: mimetype,
            caption,
            messageId,
            uploadedBy: userId,
            createdAt: new Date(),
            updatedAt: new Date(),
        };
    }
    async getTaskFiles(taskId) {
        return await this.projectMediaService.getTaskFiles(taskId);
    }
    async getProjectFiles(projectId) {
        return await this.projectMediaService.getProjectFiles(projectId);
    }
    async deleteFile(fileId, type, userId) {
        return await this.projectMediaService.deleteFile(fileId, userId, type);
    }
    async streamToBuffer(stream) {
        const chunks = [];
        return new Promise((resolve, reject) => {
            stream.on('data', (chunk) => chunks.push(Buffer.from(chunk)));
            stream.on('error', (err) => reject(err));
            stream.on('end', () => resolve(Buffer.concat(chunks)));
        });
    }
};
exports.ProjectMediaResolver = ProjectMediaResolver;
__decorate([
    (0, graphql_1.Mutation)(() => media_dto_1.ProjectMediaType, {
        name: 'uploadProjectFile',
        description: 'Upload file to project - Use Dynamic GraphQL createOne instead',
    }),
    __param(0, (0, graphql_1.Args)('projectId', { type: () => graphql_1.ID })),
    __param(1, (0, graphql_1.Args)('filename')),
    __param(2, (0, graphql_1.Args)('mimetype')),
    __param(3, (0, graphql_1.Args)('size')),
    __param(4, (0, graphql_1.Args)('url')),
    __param(5, (0, graphql_1.Args)('caption', { nullable: true })),
    __param(6, (0, current_user_decorator_1.CurrentUser)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String, Number, String, String, String]),
    __metadata("design:returntype", Promise)
], ProjectMediaResolver.prototype, "uploadProjectFile", null);
__decorate([
    (0, graphql_1.Mutation)(() => media_dto_1.ProjectMediaType, {
        name: 'uploadTaskFile',
        description: 'Upload file to task - Use Dynamic GraphQL createOne instead',
    }),
    __param(0, (0, graphql_1.Args)('taskId', { type: () => graphql_1.ID })),
    __param(1, (0, graphql_1.Args)('filename')),
    __param(2, (0, graphql_1.Args)('mimetype')),
    __param(3, (0, graphql_1.Args)('size')),
    __param(4, (0, graphql_1.Args)('url')),
    __param(5, (0, graphql_1.Args)('caption', { nullable: true })),
    __param(6, (0, current_user_decorator_1.CurrentUser)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String, Number, String, String, String]),
    __metadata("design:returntype", Promise)
], ProjectMediaResolver.prototype, "uploadTaskFile", null);
__decorate([
    (0, graphql_1.Mutation)(() => media_dto_1.ChatMediaType, {
        name: 'uploadChatFile',
        description: 'Upload file to chat message - Use Dynamic GraphQL createOne instead',
    }),
    __param(0, (0, graphql_1.Args)('messageId', { type: () => graphql_1.ID })),
    __param(1, (0, graphql_1.Args)('filename')),
    __param(2, (0, graphql_1.Args)('mimetype')),
    __param(3, (0, graphql_1.Args)('size')),
    __param(4, (0, graphql_1.Args)('url')),
    __param(5, (0, graphql_1.Args)('caption', { nullable: true })),
    __param(6, (0, current_user_decorator_1.CurrentUser)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String, Number, String, String, String]),
    __metadata("design:returntype", Promise)
], ProjectMediaResolver.prototype, "uploadChatFile", null);
__decorate([
    (0, graphql_1.Query)(() => [media_dto_1.ProjectMediaType], {
        name: 'taskFiles',
        description: 'Get all files for a task',
    }),
    __param(0, (0, graphql_1.Args)('taskId', { type: () => graphql_1.ID })),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], ProjectMediaResolver.prototype, "getTaskFiles", null);
__decorate([
    (0, graphql_1.Query)(() => [media_dto_1.ProjectMediaType], {
        name: 'projectFiles',
        description: 'Get all files for a project',
    }),
    __param(0, (0, graphql_1.Args)('projectId', { type: () => graphql_1.ID })),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], ProjectMediaResolver.prototype, "getProjectFiles", null);
__decorate([
    (0, graphql_1.Mutation)(() => Boolean, {
        name: 'deleteFile',
        description: 'Delete a file',
    }),
    __param(0, (0, graphql_1.Args)('fileId', { type: () => graphql_1.ID })),
    __param(1, (0, graphql_1.Args)('type')),
    __param(2, (0, current_user_decorator_1.CurrentUser)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String]),
    __metadata("design:returntype", Promise)
], ProjectMediaResolver.prototype, "deleteFile", null);
exports.ProjectMediaResolver = ProjectMediaResolver = __decorate([
    (0, graphql_1.Resolver)(),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __metadata("design:paramtypes", [project_media_service_1.ProjectMediaService])
], ProjectMediaResolver);
//# sourceMappingURL=project-media.resolver.js.map