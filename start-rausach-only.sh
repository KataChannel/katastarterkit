...
# Quick Start Script for Rausach Domain Only
...
echo -e "${BLUE}🚀 Starting Rausach Domain (Port 12xxx)...${NC}"
echo ""

# Start infrastructure + rausach services
docker-compose -f "$COMPOSE_FILE" up -d postgres redis minio rausach-backend rausach-frontend

echo ""
echo -e "${GREEN}✅ Rausach Domain Started!${NC}"
echo ""
...
echo -e "  docker-compose -f $COMPOSE_FILE logs -f rausach-backend rausach-frontend"
echo ""
